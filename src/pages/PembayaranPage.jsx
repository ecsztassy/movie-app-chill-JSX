...
localStorage.removeItem('premiumPlan')
  navigate('/login')
      ...
      <div style={{ padding: isMobile ? '20px 15px' : '40px', maxWidth: '800px' }}>
        ...
        <div style={{ display: 'flex', gap: '30px', alignItems: 'flex-start' }}>
          ...
          {/* Kiri - Kartu Paket */}
          <div style={{
            width: '180px', background: '#3b5bdb', borderRadius: '12px',
              ...
              <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: '30px', alignItems: 'flex-start' }}>
                {metodes.map(m => (
                  <button
                    ...
                    style={{
                      padding: '8px 14px', borderRadius: '6px', cursor: 'pointer', fontSize: '11px',
                      background: metodePembayaran === m.id ? '#3b5bdb' : '#2a2a2a',
                      border: metodePembayaran === m.id ? '2px solid #5c7cfa' : '2px solid #444',
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <input
                  ...
                  style={{ flex: 1, background: '#2a2a2a', border: '1px solid #444', borderRadius: '6px', padding: '10px 14px', color: 'white', fontSize: '13px', outline: 'none' }}
                />
                <button style={{ background: '#3b5bdb', border: 'none', color: 'white', padding: '10px 18px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: 'bold' }}>
                  Gunakan
                </button>
              </div>
            </div>
  ...
  <span>Rp{paket.nominal.toLocaleString('id-ID')}</span>
              ...
              <button
  onClick={() => navigate('/pembayaran-detail', { state: { paket, metode: metodePembayaran } })}
  style={{ width: '100%', padding: '11px', background: '#3b5bdb', border: 'none', borderRadius: '6px', color: 'white', fontWeight: 'bold', fontSize: '14px', cursor: 'pointer' }}
>
  Bayar
</button>
            </div>
