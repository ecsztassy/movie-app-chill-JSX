...
function Countdown() {
  ...
  return (
    <div style={{ background: '#1e1e2e', borderRadius: '10px', padding: '18px', textAlign: 'center', marginBottom: '30px' }}>
      <div style={{ color: '#aaa', fontSize: '13px', marginBottom: '12px' }}>Lakukan Pembayaran Before</div>
        ...
        <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: '20px', alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1, width: '100%' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px', fontSize: '13px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: '#aaa' }}>Tanggal Pembelian</span><span>08 Juni 2023</span></div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <button onClick={handleCopy} style={{ background: 'transparent', border: 'none', color: copied ? '#69db7c' : '#3b5bdb', cursor: 'pointer', fontSize: '16px' }}>{copied ? '✓' : '📋'}</button>
                </div>
              </div>
            </div>
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              <ol style={{ paddingLeft: '18px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {selectedMetode.tata.map((step, i) => <li key={i} style={{ color: '#ccc', fontSize: '12px', lineHeight: '1.5' }}>{step}</li>)}
              </ol>
            </div>

            <button onClick={() => { localStorage.setItem('isPremium', 'true'); localStorage.setItem('premiumPlan', JSON.stringify(paket)); navigate('/profile') }} style={{ width: isMobile ? '100%' : 'auto', background: '#3b5bdb', border: 'none', color: 'white', padding: '12px 28px', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', fontSize: '14px' }}>Bayar</button>
          </div>
        </div>
      </div>
