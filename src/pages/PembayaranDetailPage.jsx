...
const metodeData = {
  visa: {
    ...
    desc: 'Kartu VISA',
    tata: [
      'Masukkan nomor kartu VISA kamu.',
      'Isi tanggal kadaluarsa dan CVV.',
      'Masukkan nama pemegang kartu.',
      'Klik Bayar dan tunggu konfirmasi.',
      'Cek email untuk bukti pembayaran.',
    ],
  },
  mastercard: {
    label: 'Mastercard',
    icon: '💳',
    desc: 'Kartu Mastercard',
    tata: [
      'Masukkan nomor kartu Mastercard kamu.',
      'Isi tanggal kadaluarsa dan CVV.',
      'Masukkan nama pemegang kartu.',
      'Klik Bayar dan tunggu konfirmasi.',
      'Cek email untuk bukti pembayaran.',
  ...
  kartu: {
    ...
    tata: [
      'Masukkan nomor kartu debit atau kredit kamu.',
      'Isi tanggal kadaluarsa dan CVV.',
      'Masukkan nama pemegang kartu.',
      'Klik Bayar dan tunggu konfirmasi.',
      'Cek email untuk bukti pembayaran.',
...
localStorage.removeItem('premiumPlan')
  navigate('/login')
      ...
      <div style={{ padding: '40px', maxWidth: '800px' }}>
        ...
        <h2 style={{ fontSize: '22px', marginBottom: '24px' }}>Ringkasan Pembayaran</h2>

        <div style={{ display: 'flex', gap: '30px', alignItems: 'flex-start' }}>

          {/* Kiri - Kartu Paket */}
          <div style={{ width: '180px', background: '#3b5bdb', borderRadius: '12px', padding: '18px', flexShrink: 0 }}>
            <div style={{ display: 'inline-block', background: 'rgba(255,255,255,0.2)', borderRadius: '20px', padding: '4px 14px', fontSize: '12px', fontWeight: 'bold', marginBottom: '10px' }}>
              {paket.label}
            </div>
            ...
            <button style={{ width: '100%', padding: '8px', border: 'none', borderRadius: '6px', background: 'white', color: '#3b5bdb', fontWeight: 'bold', fontSize: '12px', cursor: 'pointer' }}>
              Langganan
            </button>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px', fontSize: '13px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span style={{ fontWeight: 'bold' }}>{kodePembayaran}</span>
                  <button onClick={handleCopy} style={{ background: 'transparent', border: 'none', color: copied ? '#69db7c' : '#3b5bdb', cursor: 'pointer', fontSize: '16px' }} title="Salin kode">
                    {copied ? '✓' : '📋'}
                  </button>
                </div>
              </div>
  ...
  localStorage.setItem('premiumPlan', JSON.stringify(paket))
  navigate('/profile')
}}
  style={{ background: '#3b5bdb', border: 'none', color: 'white', padding: '10px 28px', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', fontSize: '14px' }}
>
  Bayar
</button>
          </div>
