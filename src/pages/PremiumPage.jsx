...
const pakets = [
  { id: 'individual', label: 'Individual', harga: 'Mulai dari Rp49.990/bulan\n1 Akun', nominal: 49990, fitur: ['Tidak ada Iklan', 'Kualitas 720p', 'Download konten pilihan'] },
  { id: 'berdua', label: 'Berdua', harga: 'Mulai dari Rp79.990/bulan\n2 Akun', nominal: 79990, fitur: ['Tidak ada Iklan', 'Kualitas 1080p', 'Download konten pilihan'] },
  { id: 'keluarga', label: 'Keluarga', harga: 'Mulai dari Rp159.990/bulan\n3-7 Akun', nominal: 159990, fitur: ['Tidak ada Iklan', 'Kualitas 4K', 'Download konten pilihan'] }
]
...
function PremiumPage() {
  ...
  return (
    <div style={{ background: '#181818', color: 'white', minHeight: '100vh', fontFamily: 'Arial, sans-serif' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(3, 1fr)', gap: '20px', maxWidth: '600px', margin: '0 auto' }}>
          {features.map((f, i) => (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px' }}>
              <div style={{ width: '50px', height: '50px', borderRadius: '10px', background: '#2a2a2a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: f.is4k ? '14px' : '22px', fontWeight: f.is4k ? 'bold' : 'normal', color: f.is4k ? '#3b5bdb' : 'white' }}>{f.icon}</div>
      ...
      <div style={{ background: '#1a1a2e', padding: isMobile ? '40px 15px' : '50px 40px', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'center', gap: '16px', alignItems: isMobile ? 'center' : 'flex-start' }}>
          {pakets.map(p => (
            <div key={p.id} onClick={() => setSelectedPaket(p.id)} style={{ width: '200px', background: selectedPaket === p.id ? '#3b5bdb' : '#252540', borderRadius: '12px', padding: '20px', cursor: 'pointer', transition: '0.2s', border: selectedPaket === p.id ? '2px solid #5c7cfa' : '2px solid transparent', boxSizing: 'border-box' }}>
              <div style={{ display: 'inline-block', background: selectedPaket === p.id ? 'rgba(255,255,255,0.2)' : '#3b5bdb', borderRadius: '20px', padding: '4px 14px', fontSize: '12px', fontWeight: 'bold', marginBottom: '12px' }}>{p.label}</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px', textAlign: 'left' }}>
                {p.fitur.map((f, i) => <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px', color: '#ddd' }}><span style={{ color: '#69db7c' }}>✓</span> {f}</div>)}
              </div>
              <button onClick={() => navigate('/pembayaran', { state: { paket: p } })} style={{ width: '100%', padding: '9px', border: 'none', borderRadius: '6px', background: selectedPaket === p.id ? 'white' : '#3b5bdb', color: selectedPaket === p.id ? '#3b5bdb' : 'white', cursor: 'pointer', fontWeight: 'bold', fontSize: '13px' }}>Langganan</button>
      ...
      {/* --- RESPONSIVE FOOTER --- */}
      <footer style={{ background: '#111', padding: isMobile ? '20px 15px' : '40px', marginTop: '0' }}>
        {!isMobile ? (
